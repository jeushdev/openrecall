...
class _CompleteLocalDeckStore extends LocalDeckStore {
  _CompleteLocalDeckStore(this.localCards) : super(null);

  final List<FlashCard> localCards;
  ...
  @override
  Future<List<FlashCard>> cards(String deckId) async => localCards;
}
...
void main() {
  ...
  StudySessionState state() => container.read(sessionControllerProvider).value!;
  ...
  group('start', () {
    test(
      'complete offline package starts without awaiting a remote read',
      () async {
        final localCard = _card('offline');
        decks = FakeDeckRepository()..hangForever = true;
        study = FakeStudyRepository();
        container = ProviderContainer(
          overrides: [
            localDeckStoreProvider.overrideWithValue(
              _CompleteLocalDeckStore([localCard]),
            ),
            deckRepositoryProvider.overrideWithValue(decks),
            studyRepositoryProvider.overrideWithValue(study),
          ],
        ...
        await start().timeout(const Duration(milliseconds: 100));

        expect(state().current!.cardId, 'offline');
        expect(
          decks.calls.where((call) => call.startsWith('fetchCards')),
          isEmpty,
        );
    ...
    test(
      'parked drill starts from the complete package without remote work',
      () async {
        final localCard = _card('parked');
        decks = FakeDeckRepository()..hangForever = true;
        study = FakeStudyRepository();
        container = ProviderContainer(
          overrides: [
        ...
        await controller()
            .startParkedDrill(
              ...
              deckName: 'Biology',
              mode: StudyMode.flip,
              parkedCardIds: const ['parked'],
            )
            .timeout(const Duration(milliseconds: 100));

        expect(state().current!.cardId, 'parked');
        expect(
          decks.calls.where((call) => call.startsWith('fetchCards')),
          isEmpty,
        );
    ...
    test(
      'abandons an existing active session before creating the new one',
      () async {
        ...
        final abandonIndex = study.calls.indexWhere(
          (c) => c.startsWith('abandonActiveSessions'),
        );
        final createIndex = study.calls.indexWhere(
          (c) => c.startsWith('createSession'),
        );
    ...
    test(
      'creates a flip session and seeds only due cards with sparse positions',
      () async {
        decks = FakeDeckRepository(
          cards: [_card('a'), _card('m', mastery: 4), _card('b')],
        );
        study = FakeStudyRepository();
        container = makeContainer();
    ...
    test('applies the cap after filtering out mastered cards', () async {
      decks = FakeDeckRepository(
        cards: [_card('m', mastery: 4), _card('a'), _card('b'), _card('c')],
      );
      study = FakeStudyRepository();
      container = makeContainer();
      ...
      expect(
        study.sessionCardsFor(study.sessions.single.id).map((c) => c.cardId),
        ['a', 'b'],
      );
  ...
  group('start — CardScope.all', () {
    test('seeds a non-empty queue from a fully-mastered deck and records '
      ...
      expect(study.sessionCardsFor(session.id).map((c) => c.cardId), [
        'a',
        'b',
        'c',
      ]);
    ...
    test(
      'a due-scoped session on the same fully-mastered deck errors',
      () async {
        decks = FakeDeckRepository(
          cards: [_card('a', mastery: 4), _card('b', mastery: 4)],
        );
        study = FakeStudyRepository();
        container = makeContainer();
    ...
    test(
      'a Mastered card rated below 4 requeues and its mastery_level drops',
      () async {
        decks = FakeDeckRepository(cards: [_card('a', mastery: 4)]);
        study = FakeStudyRepository();
        container = makeContainer();
  ...
  group('rate', () {
    ...
    test(
      'advances the queue synchronously, before any write confirms',
      () async {
        await start();
    ...
    test(
      'a fail keeps the card, requeues it, and writes the session_card',
      () async {
        await start();
        controller().rate(FlipRating.forgotten);
        await pumpEventQueue();
    ...
    test(
      'a re-rate before the first write confirms does not lose the update',
      () async {
        await start();
        ...
        expect(decks.cardById('a')!.masteryLevel, 4);
        expect(
          decks.cardById('a')!.failCount,
          1,
        ); // exactly one lifetime increment
      },
    );
  ...
  group('park', () {
    ...
    test('confirmPark writes is_parked and can complete the session', () async {
      ...
      expect(study.calls, contains(startsWith('updateSessionCard(id=sc-1')));
    ...
    test(
      'declinePark resets the counter and writes consecutive_fails 0',
      () async {
        await failToPrompt();
  ...
  group('completion and exit', () {
    ...
    test(
      'completing the queue flushes mastery writes before completeSession',
      () async {
        await start();
        decks.guardGate = Completer<void>();
        ...
        expect(
          study.calls.any((c) => c.startsWith('completeSession')),
          isFalse,
        );
    ...
    test(
      'exit leaves the session active and keeps the state for resume',
      () async {
        await start();
        controller().rate(FlipRating.forgotten);
        controller().exit();

...
void main() {
  group('requeuePosition', () {
    ...
    test('with exactly 3 cards ahead, goes to the back of the queue', () {
      expect(
        requeuePosition([1000, 2000, 3000], currentPosition: 500),
        4000,
      );
    });
    ...
    test('with no cards ahead, repeats immediately after the current position', () {
      expect(requeuePosition(const [], currentPosition: 4000), 5000);
    });

    test('repeated requeues into one gap stay strictly increasing and unique',
        () {
      ...
      for (var i = 0; i < 50; i++) {
        ...
        expect(ahead.where((p) => p < next).length, greaterThanOrEqualTo(3),

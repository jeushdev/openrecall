...
void main() {
  group('clozeSegments', () {
    test('numbers a blank per keyword occurrence, threading startIndex', () {
      final (front, next) =
          clozeSegments('A cell has a cell wall', ['cell']);
      final blanks = front.where((s) => s.isBlank).toList();
      expect(blanks.map((s) => s.blankIndex), [0, 1]);
      expect(next, 2);

      final (back, end) = clozeSegments('the cell membrane', ['cell'],
          startIndex: next);
    ...
    test('among overlapping keywords the longest wins at a position', () {
      final (segs, _) =
          clozeSegments('the cell membrane', ['cell', 'cell membrane']);
  ...
  group('clozeBlankAnswers', () {
    ...
    test('preserves the cased substring the card actually blanked', () {
      expect(
        clozeBlankAnswers('The MITOCHONDRIA', 'a mitochondria', ['mitochondria']),
    ...
    test('is empty when no keyword appears in the card', () {
      expect(
        clozeBlankAnswers('front text', 'back text', ['absent']),
        isEmpty,
      );
    });
  });

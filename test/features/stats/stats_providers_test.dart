...
void main() {
  ...
  test(
    'overallMasteryProvider is the card-weighted % across every deck',
    () async {
  ...
  test('overallMasteryProvider resolves with appDatabaseProvider null', () async {
    ...
    expect(container.read(overallMasteryProvider), isA<AsyncValue<int>>());
  ...
  test(
    'recentCompletedSessionsProvider passes the repository result through',
    () async {
      final result = await container.read(
        recentCompletedSessionsProvider.future,
      );
  ...
  test('recentActivityProvider merges sessions, decks and courses, newest first', () async {
    ...
    expect(feed.map((i) => i.kind), contains(ActivityKind.deckCreated));
  });

  test(
    'deckRunThroughsProvider passes the repository result through',
    () async {
      expect(await container.read(deckRunThroughsProvider.future), {
        'd1': 3,
        'd3': 1,
      });
  ...
  test(
    'a stats read that never returns lands in error, not a forever spinner',
    () async {
      stats.hangForever = true;
      final bounded = ProviderContainer(
        overrides: [
          deckRepositoryProvider.overrideWithValue(decks),
          courseRepositoryProvider.overrideWithValue(courses),
          statsRepositoryProvider.overrideWithValue(stats),
  ...
  test(
    'historyLogProvider joins each completed session to its deck and course',
    () async {
  ...
  test(
    'dailyActivityProvider counts completed sessions per calendar day',
    () async {

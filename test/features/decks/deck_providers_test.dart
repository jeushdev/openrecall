...
void main() {
  ...
  test('the controller starts idle with a data state', () {
    expect(
      container.read(decksControllerProvider),
  ...
  test(
    'addCard forwards its fields and refreshes that deck\'s cards',
    () async {
      ...
      expect(
        fake.calls,
        contains(
          'addCard(deck=deck-1, front=Q, back=A, '
  ...
  test('addCards inserts every parsed card in one call', () async {
    await container.read(decksControllerProvider.notifier).addCards(
      'deck-1',
      const [
  ...
  test('deleteCard forwards the id and refreshes the list', () async {
    final card = await container
        .read(decksControllerProvider.notifier)
        .addCard(deckId: 'deck-1', front: 'Q', back: 'A');
  ...
  test(
    'updateDeck forwards the non-null fields and refreshes that deck',
    () async {
      final deck = await container
          .read(decksControllerProvider.notifier)
          .createDeck('Bio');
      container.listen(
        deckCardsProvider(deck!.id),
        (_, _) {},
        fireImmediately: true,
      );
      ...
      final updated = await container
          .read(decksControllerProvider.notifier)
          .updateDeck(id: deck.id, name: 'Bio 101', courseId: 'course-9');
  ...
  test('deleteDeck forwards the id and drops it from the list', () async {
    ...
    await container.read(decksControllerProvider.notifier).deleteDeck(deck!.id);

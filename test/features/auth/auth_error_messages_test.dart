...
void main() {
  group('authErrorMessage', () {
    ...
    test('maps an already-registered email', () {
      expect(
        authErrorMessage(const AuthApiException('User already registered',
            statusCode: '422', code: 'user_already_exists')),
        'An account with this email already exists.',
      );
      expect(
        authErrorMessage(const AuthApiException('Email address already in use',
    ...
    test('maps an unconfirmed email', () {
      expect(
        authErrorMessage(const AuthApiException('Email not confirmed',
    ...
    test('maps a weak password', () {
      expect(
        authErrorMessage(AuthWeakPasswordException(
    ...
    test('maps rate limiting', () {
      expect(
        authErrorMessage(const AuthApiException('Too many requests',
    ...
    test('maps dart:io and http transport errors to a connectivity message', () {
      const connectivity =
          'Can\'t reach the server. Check your connection and try again.';
      expect(
          authErrorMessage(const SocketException('Failed host lookup')),
          connectivity);
      expect(authErrorMessage(http.ClientException('Connection closed')),
          connectivity);
    });

    test('falls back to a generic message for an unknown AuthException code', () {
      expect(
        authErrorMessage(

...
void main() {
  ...
  test('a fresh v6 database has the reorder columns and offline_meta', () async {
    ...
    await db.insert('offline_courses',
        {'id': 'c1', 'name': 'Bio', 'accent_color': 'green'});
    ...
    final deck = (await db.query('offline_decks', where: 'id = ?',
        whereArgs: ['d1'])).single;
    final course = (await db.query('offline_courses', where: 'id = ?',
        whereArgs: ['c1'])).single;
    final card = (await db.query('offline_cards', where: 'id = ?',
        whereArgs: ['k1'])).single;
    expect(deck['position'], 0);
    expect(course['position'], 0);
    expect(card['created_locally'], 0);

    await db.insert('offline_meta', {'key': 'last_user_id', 'value': 'user-a'});
    final meta = (await db.query('offline_meta', where: 'key = ?',
        whereArgs: ['last_user_id'])).single;

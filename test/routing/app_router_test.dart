    ...
    ProviderScope(
      overrides: [
        ...
        deckRepositoryProvider.overrideWithValue(
          FakeDeckRepository(cards: [_masteredCard('a')]),
        ),
...
void main() {
  ...
  testWidgets(
    'the shell has four branches in order: Home, Decks, History, More',
    (tester) async {
      await _pump(tester, signedIn: true);
  ...
  testWidgets('tab clearance matches the rendered navigation obstruction', (
    tester,
  ) async {
    await _pump(tester, signedIn: true);

    final barHeight = tester.getSize(find.byType(IosTabBar)).height;
    final clearance = tester
        .getSize(find.byKey(const ValueKey('large-title-bottom-clearance')))
        .height;
    expect(clearance, barHeight + 16);
  });

  testWidgets('standalone Settings does not inherit tab-bar clearance', (
    tester,
  ) async {
    await _pump(tester, signedIn: true);
    final tabClearance = tester
        .getSize(find.byKey(const ValueKey('large-title-bottom-clearance')))
        .height;
    ...
    final settingsClearance = tester
        .getSize(find.byKey(const ValueKey('large-title-bottom-clearance')))
        .height;
    expect(find.byType(IosTabBar), findsNothing);
    expect(settingsClearance, lessThan(tabClearance));
    expect(settingsClearance, 16);
  });

  testWidgets(
    'switching tabs preserves the inactive branch in the IndexedStack',
    (tester) async {
      await _pump(tester, signedIn: true);
      ...
      expect(
        find.byType(HistoryTabScreen, skipOffstage: false),
        findsOneWidget,
      );
    },
  );
  ...
  testWidgets(
    'pushing /deck-creator leaves the shell and pops back to its tab',
    (tester) async {
      await _pump(tester, signedIn: true);
      final router = _router(tester);

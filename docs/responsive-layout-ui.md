...
**Execution baseline:** Static inspection confirmed the fixed 260-pixel carousel, inner 0.72 aspect ratio, 56-pixel unfinished rows, 184-pixel strip reservation, 34-pixel percentage allocation, and two-column square grid. The strongest focused reproduction measured a 209-pixel gap from a lone unfinished session's percentage to the next section because the strip reserved all three rows. Milestones 2 and 3 were complete, and their navigation clearance, header, row, and selector conventions were preserved.

#### Layout policy and boundaries

- **Carousel:** Give the bounded `PageView` one height owner based on its local available width and the tallest displayed card's scaled content. Account for the 0.82 page fraction, 8-pixel horizontal page padding per side, 18-pixel card padding, course heading/underline, deck title, and footer. Keep 260 pixels as the existing default extent where content fits; grow when required and remove the competing inner aspect-ratio constraint. Measure with the effective styles and text scaler, including button padding/minimum size. Reserve the front-card footer requirement for every page so paging does not change the stack height. Recompute when width, text scale, or data changes; do not measure on every animation tick.
- Preserve the existing initial middle page, finite paging, neighboring cards, scale/opacity/blur transitions, rounding, and front-only View Deck action. Keep count and View Deck in one row when they fit; otherwise wrap/reflow the footer in its existing reading order with a complete action label. The outer Home scroll view must make a taller card and its action reachable; do not add an inner card scroll area or reduce fonts.
- **Names:** Audit existing Home and grid summary ellipsis against actual full-value access. Retain intentional truncation only where the complete course/deck name is discoverably accessible by touch, including locked-offline decks. Otherwise let names wrap and include their full height in sizing. A semantics label alone is insufficient. Prefer wrapping within this layout scope rather than adding new navigation or dialogs. Resolve this policy before finalizing measurements so tests and rendered text use the same assumptions.
- **Unfinished sessions:** The real Home caller hides the entire section when there are no sessions; preserve that behavior. The private strip's empty fallback must not reserve three rows. For populated content, use a naturally sized column containing only the first three sessions and intervening 8-pixel gaps. Treat 56 pixels as a row minimum, allocate the percentage from its actual text width, and let rows grow. Preserve the blue accent, progress track, percentage formatting, session order, and resume arguments. If a single row leaves inadequate space for the name/progress, move progress and percentage beneath the name while preserving reading order. Keep progress visible and percentages complete, including 100% at scale 2.0.
- **Deck grids:** Use local grid constraints to compute column width after the existing 14-pixel gap. Retain two square columns where content fits; otherwise increase a common tile height to accommodate the title, 8-pixel gap, badge/offline hint, 12-pixel content padding, and 8-pixel backing inset. Prefer taller two-column tiles before reducing columns. Use a one-column fallback only if measured minimum usable content/control width requires it, never a device-name breakpoint. Include the Create footer in the sizing requirements. Verify which sizing arguments the installed reorderable-grid package supports before choosing a main-axis extent or derived aspect ratio.
- Bound badge and offline-hint text so it wraps completely within the foreground card. Preserve empty/populated badge wording, accent colors, left backing sliver, offline opacity and snackbar behavior. Keep stable deck keys, existing optimistic reorder/persistence calls, non-draggable Create footer, and outer accordion scroll ownership. Do not introduce intrinsic measurement across the entire scrolling screen or rewrite it into slivers.
- Production scope begins with the four scope files above. `create_deck_tile.dart` and the Decks accordion caller may change only for a demonstrated integration requirement. Small layout helpers may be extracted if they keep sizing and rendering consistent. Home mastery content, providers, navigation/header redesign, study layouts, theme-token cleanup, and business logic remain outside scope; document any newly discovered dependency.

#### Execution sequence after the go signal

1. **Establish the baseline.** Reinspect working-tree changes, actual caller padding, theme/fonts, grid-package API, and the existing Home/tile/Decks tests. Run the focused existing suites and capture default-scale light/dark appearance. Recheck the historical four stale Home expectations individually; update only assertions contradicted by verified current behavior in this milestone's tests. Do not remove intentional carousel neighbors to satisfy an old single-widget finder. Leave unrelated palette expectations outside scope.

2. **Reproduce before fixing.** Start at 320×568/scale 2.0 with long course/deck names and large card counts. Inspect front-card footer bounds and vertical containment, a one-session strip with 100%, and real two-column tiles with empty/populated/offline states. Record actual overflow, clipping, or unused-slot geometry. Use focused failing regressions for confirmed defects; if a candidate passes, retain its implementation and fill missing acceptance coverage without inventing a failure.

3. **Implement Home sizing.** Resolve full-name access, establish the carousel's single measured height, and reflow the footer only when needed. Make unfinished content and percentages naturally sized using the row policy above. Preserve empty/loading/error visibility decisions. Verify paging to the first and last deck, front-action hit testing, resume route arguments, and vertical scrolling before proceeding to grid changes.

4. **Implement grid sizing and integration.** Derive a shared tile extent from actual usable width and rendered text needs; make badges/offline hints respect it. Keep the Create tile usable in an otherwise empty grid. Exercise expansion/collapse and long-press reordering through the real Decks screen, including drag positions after tile heights grow. Verify persisted deck order, fixed footer behavior, navigation, and offline feedback without changing their logic.

5. **Validate and record Milestone 4 only.** Complete the matrix and interaction checks below, run relevant existing suites and analysis for changed code, and inspect representative renders. Perform an Android font-scaling smoke pass if an authenticated target is available. Update milestone status and append actual results and limitations to the execution log only after execution. Stop after Milestone 4.

#### Validation and review checklist

| Area | Cases and required evidence |
| --- | --- |
| Sizing matrix | All six viewport pairs in the validation contract × text scales 1.0, 1.3, 1.5, and 2.0, inside real caller padding. Load bundled fonts where geometry depends on them; reset view size, pixel ratio, scaler, and insets between cases. Assert text/control containment and scroll reachability as well as absence of exceptions. Include runtime width and scale changes. |
| Home carousel | Zero, one, and the current maximum six decks; absent/short/long course names; spaced and unbroken deck names; singular and large counts. Verify complete footer labels, stable height while paging, first/middle/last pages, front-only action behavior, correct deck routing, and neighbors that do not obscure the active action. Include data refresh without changing provider ordering or limits. |
| Unfinished sessions | Zero, one, two, three, and more than three inputs; verify hidden empty section, exactly the first three displayed sessions, and no unused row reservation. Cover 0%, intermediate values, and 100%, long names, variable row growth, visible progress, and correct deck/mode resume arguments. |
| Grid primitives | Empty and large-count badges, long/unbroken titles, locked-offline hint, Create-only and mixed grids. Verify readable full labels or proven full-name access, foreground/backing geometry, default two-column squares where usable, increased height/fallback where required, and complete tappable bounds. |
| Full screens and interactions | Home and Decks at all target widths and short/tall heights with representative top/bottom system insets. Include populated, empty, loading, and error states supported by each screen; scroll the final action clear of existing navigation. Verify course expansion/collapse, deck taps, Create routing, offline snackbar, and reorder results in the enlarged layout. Use route stubs to isolate later-milestone study failures. |
| Appearance and environment | Inspect default-scale light/dark renders and narrow scale-2.0 renders. Preserve typography, colors, spacing character, decorations, backing layers, and transitions. Distinguish widget/render evidence from Android system-font validation; report signed-out or unavailable-device limitations explicitly. |

Expected test touchpoints:

- Extend `test/features/home/home_tab_screen_test.dart`, `test/features/decks/deck_grid_tile_test.dart`, and `test/features/decks/decks_tab_screen_test.dart`.
- Add focused `test/features/decks/deck_grid_test.dart` and/or `test/features/decks/deck_badge_test.dart` only where they make sizing and reorder coverage clearer. Test private Home widgets through their screen/provider fixtures unless a small extraction is justified by the implementation.
- Reuse existing font/provider/view helpers; leave broad harness consolidation to Milestone 6. Do not rely solely on oversized test surfaces or exception-only assertions.

Completion requires the original acceptance criteria plus geometry, full-label access, and interaction evidence. Historical analysis warnings and suite failures must be rerun or identified as historical, not presented as current results. Record unrelated failures with reproduction conditions instead of expanding scope.
...
**Current baseline:** Milestones 1–4 are recorded as complete. Static inspection confirms the rating row still uses four equal-flex buttons, 10-pixel gaps, fixed 60-pixel height, and single-line ellipsis; both pickers still use centered non-scrollable columns and fixed 52-pixel choices. The active session already centers its card in a bounded scroll viewport. These are implementation observations, not newly reproduced failures. The Milestone 3 log records a 98-pixel deck-detail pre-session overflow at 320×568/scale 2.0; reproduce it again because subsequent changes may affect it. At planning time the only working-tree entry was untracked `.claude/`; preserve it and inspect the tree again before execution. No tests or device checks were run for this plan.

#### Layout ownership and boundaries

- **Ratings:** Retain four equal-width controls in one row wherever their complete labels fit, with the current order, colors, borders, disabled opacity, press animation, and callbacks. Replace fixed height with a 60-pixel minimum and enough vertical padding for wrapping text. Remove essential-label ellipsis and line limits. Determine fit from local width after the actual session's 16-pixel side padding, gaps, label styles, and effective text scaler. If wrapping would break words into unreadable fragments, use a two-column layout in declaration order with equal control widths and consistent row heights. Use measured content requirements, not a device breakpoint. Preserve the existing default 60-pixel appearance where it fits.
- **Pre-session choices:** Give each picker one bounded vertical scroll area that centers its content when short and scrolls when taller than the available viewport, following the existing active-card `LayoutBuilder`/`SingleChildScrollView`/minimum-height convention. Keep 24-pixel padding, content order, spacing, and choice styling. Convert 52-pixel choice heights to minimums; allow complete mode labels and the timer's duration/default annotation to wrap or reflow. Keep safe-area ownership with the containing screen and consume keyboard/system clearance once. A small shared picker body is appropriate only if it removes the same layout duplication without introducing a general framework.
- **Real callers:** Validate `ModePicker` in both `StudySessionScreen` and `DeckDetailScreen`. Deck detail gives it the space above View Cards, so full-screen tests must include that footer and the app bar. Start with the shared picker fix; change the deck-detail caller only if a reproduced local constraint issue remains. Preserve mode filtering/order, single-mode bypass, forwarded mode and scope arguments, the timer presets 30/60/90/120 seconds, the 60-second default annotation, and when the session is created.
- **Active study:** Preserve the card's existing scroll area, centering, horizontal padding, keyed transitions, backing layers, and gesture ownership. Account for the taller rating area when it appears after Feynman completion; the remaining card viewport must stay usable on short screens. Inspect the close/counter row, Feynman's prompt/timer header and reveal action, and Cloze's field/submit/feedback layout. Fix only reproduced local constraint failures with bounded wrapping or reflow. Avoid adding competing card scroll views or changing session state, timer logic, grading, queue order, persistence, or navigation.
- **Scaling policy:** Keep the existing scoped preference semantics: card text uses small 0.9, medium 1.0, large 1.15, or xlarge 1.3 and replaces the inherited scaler; the header, ratings, and pre-session pickers use the inherited system scaler. Exercise both independently and in combined screen cases. Do not multiply the scalers, remove the preference override, or claim the card honors Android system font scaling. Record this existing accessibility limitation explicitly; any change to that policy requires a separately documented decision.
- Production scope starts with the four original scope files. `deck_detail_screen.dart`, `feynman_card_view.dart`, `cloze_type_card.dart`, `flip_card.dart`, and `feynman_reference_dialog.dart` may change only for a demonstrated integration or local layout requirement. Inspect reference-dialog scrolling as part of reveal reachability. Summary redesign, unrelated deck-detail content, global typography, and Milestone 6 harness consolidation remain outside scope; record newly discovered dependencies.

#### Execution sequence after the go signal

1. **Establish the baseline.** Inspect the working tree, current study callers, theme/font setup, preference provider, and existing tests. Run the focused existing suites below and capture representative default-scale light/dark appearance. Treat previously recorded analysis warnings and palette-test failures as historical until rerun.

2. **Reproduce the strongest failures before editing.** Start with the real padded rating row at 320×568/scale 2.0 and assert full label layout, not just text-widget presence. Exercise all timer choices and the full mode list in their real available heights, including deck detail's View Cards footer. Record overflow, clipping, or unreachable controls in focused regressions. Probe long active cards at xlarge preference and system scale 2.0, Feynman before/after completion, and Cloze with keyboard insets. Retain implementations that pass rather than inventing defects.

3. **Implement control and picker sizing.** Apply content-driven rating heights and only the necessary column fallback, then add centered scrolling and growing picker choices. Keep styling and selection semantics intact. Verify each rating callback and every mode/preset selection before proceeding to active-session integration.

4. **Resolve demonstrated integration gaps.** Verify that a taller rating area leaves long Flip and Feynman content reachable, including during card transitions. Correct only reproduced local header/action constraints. In Cloze, scroll to the focused field, submit, inspect feedback, and reach the next action above the keyboard; preserve auto-derived outcomes and the absence of a rating row. Check Feynman start, early finish, timer expiry, reference open/close, and rating visibility without changing their timing or state behavior.

5. **Validate and record Milestone 5 only.** Complete the matrix and interaction evidence below, run relevant existing tests and analysis appropriate to changed code, and inspect representative renders. Perform an Android smoke pass when an authenticated target is available. Update status and append actual evidence and limitations to the execution log only after execution. Stop before Milestone 6.

#### Validation and review checklist

| Area | Cases and required evidence |
| --- | --- |
| Shared controls | All six viewport pairs × system text scales 1.0, 1.3, 1.5, and 2.0, inside actual caller padding. Load bundled fonts for geometry. Assert complete laid-out labels, text/control containment, minimum hit areas, reading order, enabled/disabled callbacks, and no layout exceptions. Include runtime width/scale changes and reset view configuration between tests. |
| Pickers and pre-session callers | All target widths and short/tall heights; supported mode subsets and all modes, every timer preset, and deck detail's View Cards action. Verify centered short content, scroll reachability of the last choice, full default annotation, safe insets, and correct selected values/routes. Cover existing loading, error, empty, single-mode bypass, forwarded-mode, and resume behavior through screen fixtures. |
| Active cards and preferences | All target widths with long front/back/reference content, spaced and unbroken text, and representative system insets. Cover all four card-size presets, combining xlarge with system scale 2.0 at both short viewports and a tall viewport. Assert the actual scoped scaler separately from outer controls, complete scroll reachability, and a usable card viewport after ratings appear. |
| Flip and transitions | Disabled ratings before reveal; all enabled rating values; tap reveal using both flip3d and fade settings; left/right rating swipes only after reveal; vertical scrolling without accidental ratings. Check intermediate and settled card-to-card frames, outgoing layers, and next-card state reset with long content. |
| Feynman | Ready, running, early finish, expiry, and finished states; full prompt/timer labels; scroll to Start/Finished/Reveal reference; open and scroll a long reference, then dismiss. Verify ratings appear only at completion and remain reachable as the card viewport shrinks. Use controlled timer advancement rather than settling an active periodic timer. |
| Cloze and keyboard | Short/tall screens with keyboard closed/open and representative top/bottom insets; long prompts, multiple blanks, answer entry, submit, correct/incorrect feedback, and subsequent actions. Assert focused field and action bounds above the keyboard after scrolling, supported outcome callbacks, and no rating row. |
| Appearance and environment | Default-scale light/dark renders and narrow scale-2.0 renders; preserve typography, accents, borders, backing layers, transitions, and content order. On Android, check system scales 1.0/2.0, small/xlarge card preferences, picker scrolling, a long active card, and Cloze keyboard behavior. Record unavailable/authentication-limited checks and distinguish widget evidence from device evidence. |

Expected test touchpoints:

- Extend `test/features/study/widgets/rating_row_test.dart` and add focused `test/features/study/widgets/mode_picker_test.dart` and `test/features/study/widgets/feynman_timer_picker_test.dart`.
- Extend `test/features/study/study_session_screen_test.dart` and `test/features/decks/deck_detail_screen_test.dart` for real caller constraints and routing; reuse current provider fixtures.
- Reuse `test/features/study/widgets/flip_card_test.dart`, `test/features/study/cloze_type_card_test.dart`, and `test/features/study/feynman_mode_test.dart` as relevant functional baselines. Add a focused Feynman view test only if needed for timer/header/reference geometry that screen fixtures cannot clearly cover.
- Retain preference-scope checks in the study screen tests and run `test/features/settings/study_appearance_preferences_test.dart` and `test/features/settings/study_appearance_provider_test.dart` where integration warrants it. Keep broad harness work in Milestone 6.

Completion requires the original acceptance criteria plus full-label geometry, scrolling, keyboard, and interaction evidence. A present text widget or an exception-free frame alone is insufficient. Record remaining failures with reproduction conditions instead of enlarging test surfaces, changing scaling semantics, or silently expanding scope.
...
**Current baseline:** Milestones 1–5 are recorded as complete. Static inspection found repeated viewport/scale setup in their regression suites and existing repository/provider fakes in `test/support/`. `pump_app.dart` already hosts the real app with fake repositories and connectivity; preserve its existing callers and pre-settle behavior. No shared font loader or `test/flutter_test_config.dart` currently exists. More uses a 1000×2200 test surface, import selection 1200×3000, and deck overview 1200×3200; these functional tests are not phone-layout evidence. Session summary has completion/routing coverage in the study screen suite, but needs explicit responsive geometry coverage. The only working-tree entry at planning time was untracked `.claude/`; preserve it and reinspect before execution. No tests or device checks were run during planning.

#### Harness design and scope

- Add a small test-only helper, provisionally `test/support/responsive_test_harness.dart`, with the six named logical viewport pairs and four text scales from the validation contract. Expose view configuration and a reusable app-builder wrapper separately so both `MaterialApp` and `MaterialApp.router` fixtures retain their real routes, provider overrides, and scaffold ownership. Reuse `pump_app.dart` and existing fakes; do not create a universal fixture that supplies hidden feature state or replaces the actual navigation shell.
- Default to device-pixel ratio 1 and explicitly convert logical dimensions/insets if another ratio is requested. Preserve view-derived `MediaQuery` data while changing only the requested scaler; do not replace it with an empty `MediaQueryData`. Model top/bottom safe padding and keyboard obstruction consistently, including the bottom padding consumed by an open keyboard. Reset every modified view property at teardown, and unmount/dispose fixtures between cases. Include a focused non-unit-ratio equivalence check and a sequential configuration/reset check because incorrect harness geometry would invalidate the matrix.
- Load the bundled Figtree and Inter regular/italic assets under their declared family names before responsive text measurements. Prefer an explicit cached test-support loader invoked by responsive suites; avoid silently changing all unrelated tests through global configuration. Verify the loader actually completes before pumping. Record that font-backed widget rendering still differs from Android rendering.
- Share narrowly useful assertions for complete essential-label layout, containment in the relevant control, and reachability inside the visible scroll viewport. Inspect paragraph truncation/line limits and actual rectangles where appropriate; text presence and `takeException()` alone are insufficient. Scroll through the actual owning scrollable with bounded attempts, then verify hit testing and tap the action. Account for safe areas, keyboard, pinned headers, and the navigation overlay when calculating visible bounds.
- Keep content-specific assertions and provider fixtures in their feature suites. Preserve tall-surface functional tests and existing callback, routing, timer, and persistence coverage. Migrate repeated responsive setup incrementally without deleting stronger assertions or multiplying every feature state across all dimensions.
- Production code is not a planned deliverable. If new coverage reproduces a local responsive defect, first retain a failing regression and record its owning milestone and exact conditions. A narrowly required layout correction can accompany coverage after the go signal under the existing constraints; broader redesign, business changes, scaling-policy changes, and unrelated cleanup require a separate follow-up. Never weaken assertions or enlarge the viewport to make a failing phone case pass.

#### Execution sequence after the go signal

1. **Inventory and establish a fresh baseline.** Inspect current changes and Milestones 1–5 implementations/tests. Map each audited root cause and acceptance criterion to an existing test or a named coverage gap. Run relevant existing suites before refactoring. Treat the historically recorded two palette expectation failures and three Home analysis warnings as unverified until rerun; report current results without modifying palette tokens or unrelated code to obtain a green run.

2. **Build and verify the shared harness.** Introduce the view/scale/inset helpers, font loader, and minimal geometry utilities. Check logical dimensions, effective descendant media data, reset isolation, and font loading. Migrate the bounded-sheet and navigation/header matrices first, comparing case names, assertions, and results before migrating rows, segmented controls, selectors, grids, ratings, and pickers. Font-backed measurement may reveal previously hidden failures; reproduce and classify them before changing layouts.

3. **Complete shared-layout coverage.** Run each affected shared primitive through all 24 viewport/scale combinations with representative long content and system insets. Keep full matrices for the sheet body, navigation/header, shared rows, settings segments, course selector, deck grid sizing, ratings, and both pre-session pickers. Retain the existing Home/card matrix where it exercises content-dependent sizing. Add keyboard, dark-theme, and runtime configuration cases selectively as specified below rather than applying a full Cartesian product to every state.

4. **Fill real-screen gaps.** Add phone-sized cases to auth, More, import selection, creator/edit flows, and session summary first, then close gaps in Home, Decks, History, Settings, and active study coverage. Exercise every listed screen family at all six target widths using a representative state at scale 1.0. Add both short viewports at scale 2.0 and a tall scale-2.0 case; use the shared matrices for intermediate scales unless a screen has unique geometry needing additional cases. Keep real shell/route constraints for integration assertions. Reuse focused tests that already meet these requirements.

5. **Inspect appearance and Android behavior.** Capture and inspect representative font-backed renders at 393×873/scale 1.0 and 320×568/scale 2.0 in light and dark themes, including a sheet, collapsed header/navigation, Home carousel/grid, keyboard form, active card, and summary. Use deterministic animation frames for screenshots; do not introduce a broad golden-baseline update. Check Android availability and authentication separately, perform the smoke pass below where possible, and record skipped checks precisely.

6. **Validate and report Milestone 6 only.** Run migrated and added suites, the relevant existing feature/routing suites, then the full `flutter test` suite and `flutter analyze`. Format changed Dart files. Investigate new failures and repeat checks only where changes warrant it. Update the coverage inventory with actual test names and the execution log with commands/results, visual evidence, Android environment, and remaining issues. Do not mark complete while a known in-scope reachability/clipping failure remains in an exercised flow; record blocked coverage or unavailable manual checks explicitly rather than claiming they passed.

#### Coverage and evidence checklist

| Area | Required cases and observable outcomes |
| --- | --- |
| Root-cause inventory | Link sheet height/keyboard ownership (M1), navigation/header/clearance (M2), rows/segments/course labels (M3), Home session/carousel/grid sizing (M4), and ratings/pickers/active study integration (M5) to named regressions. Include newly probed auth/import/summary gaps and record their outcomes. |
| Navigation and tab screens | Long Home greetings; offline/pending Decks actions; expanded, intermediate, and collapsed headers; all tab taps; last actions clear of the actual navigation overlay. Include empty/populated Home and Decks, zero/one/three unfinished sessions, long deck/course names, large counts, History metadata, and More/standalone Settings without duplicate clearance. Use supported loading/error states in focused cases. |
| Sheets, creators, and import | Expanded Create Menu with the last long-named target selected; course/deck/profile editing with validation and reachable Save; course/deck creator labels and selection; actual import screen empty/populated/error states and final selection/import action. At 320×568 and 360×640/scale 2.0, use representative 240–300 logical-pixel keyboard insets for text-entry flows and verify focus, scrolling, submit, and keyboard closing. Preserve existing disabled/busy behavior. |
| Auth | Login and sign-up through existing fake-auth fixtures at phone sizes, long validation/error feedback, keyboard closed/open, reachable fields, primary submit and secondary navigation. Verify expected fake repository calls and routes without real account creation or network dependencies. |
| Pre-session and active study | All mode and timer choices in study and deck-detail callers, including View Cards below the picker. Long Flip/Feynman/Cloze content, reveal/submit/rating actions, Feynman running/finished/reference states, and Cloze focused fields and feedback above the keyboard. Exercise all four card preferences and combine xlarge with outer scale 2.0; assert that the existing card scaler replaces the system scaler while outer controls retain it. |
| Summary | Add focused `test/features/study/widgets/session_summary_view_test.dart` if the screen fixture cannot clearly cover its geometry. Include all mode-specific metric labels, long deck names, large valid counts, parked/non-parked outcomes, initial/intermediate/settled animation frames, and scrolling to Drill parked cards now and Done. Retain real completion, drill, and return-navigation checks in the study screen suite. |
| Intentional layers and gestures | Verify the active Home carousel card can be read and tapped while neighboring cards remain; cover paging and route outcomes. During study transitions, verify incoming/outgoing layers do not intercept active controls, vertical scrolling does not rate a card, and post-reveal swipes preserve existing outcomes. Use controlled pumps rather than settling periodic timers. |
| Themes and visuals | Explicit light/dark cases for each screen family, with representative renders inspected at default and enlarged scales. Check fonts, text roles, full essential labels, decoration, focus, and control overlap. Scope geometry assertions to active content so intentionally visible neighboring cards are not treated as duplicate data. |
| Android smoke pass | Record target/API, logical size, system font scale, theme, and keyboard used. At scales 1.0 and 2.0, check auth, tab navigation/header collapse, Home/Decks long content, sheet editing/import selection, Settings, pickers, long active cards, Cloze keyboard, and summary actions; cover light/dark and small/xlarge card preferences. Use existing accessible test data and restore changed device settings afterward. If no target or authenticated session is available, distinguish completed signed-out checks from unverified authenticated flows; widget insets are not proof of real IME behavior. |

Expected touchpoints are the existing responsive suites under `test/ui/common/`, `test/ui/settings/`, `test/routing/`, and `test/features/{home,decks,stats,settings,profile,study}/`, plus `test/features/auth/login_screen_test.dart`, `test/features/courses/course_creator_screen_test.dart`, `test/features/decks/import_cards_screen_test.dart`, the proposed summary test, and test-support helpers. Add phone coverage to `deck_overview_screen_test.dart` where its existing oversized fixture intersects the exercised deck flow. Keep an explicit list of executed files and coverage gaps in the completion evidence rather than interpreting these directories as permission to rewrite unrelated tests.

Completion requires the original acceptance criteria, preserved functional coverage after consolidation, and a traceable distinction between automated assertions, inspected renders, and device observations. Historical test counts and manual passes remain historical; this milestone must report its own results.
...
| 4 | 2026-09-07 | Reproduced a 209-pixel gap after a lone unfinished session because Home reserved all three session slots. Home now hides the empty section, sizes one to three rows from their content, gives percentages their measured width, and reflows narrow rows while preserving the three-session limit and resume behavior. The carousel has one width- and scaled-text-driven height owner, complete wrapping names, and a footer that reflows count and View Deck when required; paging, neighboring-card effects, and deck routing remain intact. Deck grids retain two columns at every target width while deriving a common height from complete title, badge, and offline-hint text; the Create footer, offline behavior, expansion, routing, and persisted drag reorder remain intact. Added six-viewport × four-scale Home and grid coverage, containment assertions, long names, large counts, zero/one/three-session states, paging/routing, and light/dark cases. All 38 focused tests passed. Full suite: 717 passed with two unrelated historical palette-expectation failures. The Android debug build installed and launched on an API 37 emulator; authenticated Home and Decks rendered without visible clipping at system font scales 1.0 and 2.0. | Full analysis has no new Milestone 4 findings and retains three pre-existing Home warnings for an unused style parameter and two unused private widgets. The full-suite failures remain the two stale `AppTokens.light` palette expectations; no theme tokens changed. Android dark-theme rendering was covered by widget tests rather than the manual emulator smoke pass. |
| 5 | 2026-09-07 | Reproduced essential rating labels constrained to one-line ellipsis at 320×568/scale 2.0, a 342-pixel timer-picker overflow, and a 244-pixel mode-picker overflow. Ratings now retain their four-column default when complete labels fit, grow from a 60-pixel minimum, and reflow to two columns from measured label width when required. Mode and timer choices share a centered, bounded scroll fallback and grow from their existing 52-pixel minimum; timer annotations wrap without truncation. The existing active-card scroll owner, card transition behavior, rating semantics, picker routing, timer state, and scoped card-size preference semantics remain intact. Added six-viewport × four-scale geometry/reachability coverage for ratings and both pickers, narrow dark-theme rendering, real deck-detail and study-session scale-2 integrations, and a 320×568 Cloze keyboard case with the xlarge preference. All 71 focused tests passed. Full suite: 721 passed with the same two unrelated historical palette-expectation failures. Analysis has no Milestone 5 findings and retains the three pre-existing Home warnings. | No Android device or emulator was connected, so Android system font scaling and keyboard behavior were validated with widget tests rather than a manual smoke pass. The card surface still intentionally replaces the inherited system scaler with the selected 0.9–1.3 preference scaler; changing that existing accessibility policy remains outside this milestone. |
| 6 | 2026-09-07 | Consolidated the six logical viewport sizes, four text scales, safe/keyboard insets, real-app system scaling, bundled Figtree/Inter loading, view reset, complete-label, containment, and visible-bounds helpers in `test/support/responsive_test_harness.dart`. Migrated the sheet, tab bar, collapsing header, shared row, settings segment, course selector, deck grid, rating, mode-picker, and timer-picker matrices to that harness. Added phone-sized reachability and interaction coverage for login/sign-up, course creation, import entry/footer, More, deck overview, and every summary mode, including short 2× keyboard cases and light/dark rendering. The new summary matrix reproduced a 2.7-pixel metric-row overflow at 320×568/scale 2.0 with large counts; summary values now move below their labels only when the single row cannot fit. Focused batches passed (110 screen/harness tests and 62 shared-layout/harness tests), and the final summary/harness rerun passed 17 tests. Full suite: 795 passed with two unrelated historical palette-expectation failures. Analysis has no Milestone 6 findings and retains the three pre-existing Home warnings. | No Android device or emulator was connected, so the manual Android system-font, IME, authenticated-flow, and dark-theme smoke pass could not be run. A temporary image-capture probe stalled without producing an artifact; appearance evidence is limited to deterministic light/dark widget renders and geometry assertions. The full-suite failures remain the two stale `AppTokens.light` expectations; no theme tokens changed. The card surface still intentionally replaces the inherited system scaler with the selected card-size scaler, as recorded in Milestone 5. |

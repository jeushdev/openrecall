...
/// branch.
///
/// Greeting header, the reused Overall Mastery card, a horizontal strip of
/// unfinished sessions (tap resumes), and a layered stack of the most-reviewed
/// decks (tap opens deck detail). Every value comes from a provider that
/// degrades cleanly offline; nothing here is on the study path.
      ...
      body: SafeArea(
        ...
        child: ListView(
          ...
          children: [
            ...
            // Unfinished sessions — hidden entirely when there are none, so the
            // screen doesn't carry an empty heading.
            ...active.maybeWhen(
              orElse: () => const <Widget>[],
...
class _GreetingHeader extends StatelessWidget {
  const _GreetingHeader({required this.email});
...
class _SectionHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final tokens = Theme.of(context).extension<AppTokens>()!;
    return Text(text, style: AppType.title.copyWith(color: tokens.textPrimary));
  }
}
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: accent.fill, width: 1.5),
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
